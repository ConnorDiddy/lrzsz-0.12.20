...
int from_cu(void)
{
	...
	if (major(a.st_rdev) != major(b.st_rdev) || minor(a.st_rdev) != minor(b.st_rdev))
		Fromcu = 1;
	else if (major(a.st_dev) != major(b.st_dev) || minor(a.st_dev) != minor(b.st_dev))
		Fromcu = 1;
	else
		Fromcu = 0;
#else
...
{
	printf("CALLED HERE with %d, %d", fd, n);
